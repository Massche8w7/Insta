...
bot = telebot.TeleBot('7087308540:AAFpA3k-76lgJC8rgveUVRZDb4GOow3AbQQ')
...
def start_attack_reply(message, target, port, time):
    ...
    #bot.reply_to(message, response)
    response = f"💥 𝐀𝐓𝐓𝐀𝐂𝐊 𝐁𝐘 : {username}\n𝐀𝐓𝐓𝐀𝐂𝐊 𝐒𝐓𝐀𝐑𝐓𝐄𝐃 𝐁𝐀𝐁𝐘🔥\n🎯𝐓𝐚𝐫𝐠𝐞𝐭: {target}\n🔎𝐏𝐨𝐫𝐭: {port}\n⚙️𝐓𝐢𝐦𝐞: {time} 𝐒𝐞𝐜𝐨𝐧𝐝𝐬\n🤡𝐎𝐰𝐧𝐞𝐫 : ᎢʜᴇᏒᴇᴀʟᏞᴜɪs ✘ [ Loss 0.5k ]"
    bot.reply_to(message, response)
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            ...
            if time > 180:
                ...
                record_command_logs(user_id, '/attack', target, port, time) #cmd change kr lena 😁 [ line : 251 and 256
